...
exports.get_outcome = function get_outcome(game) {
    var possible_outcomes;
    if (game.character.is_threatened === true &&
        game.action !== "A3." &&
        game.action !== "Apologize." &&
        game.action !== "ATTACK" &&
        game.action !== "Bribe the dog with a fish." &&
        game.action !== "Challenge Lord Carlos to a game of chess." &&
        game.action !== "E4." &&
        game.action !== "Enter the void." &&
        game.action !== "Grovel." &&
        game.action !== "Leave in a puff." &&
        game.action !== "Make it hard for Lord Carlos to kill you." &&
        game.action !== "Nf3." &&
        game.action !== "Panic!" &&
        game.action !== "Play dead." &&
        game.action !== "Repay your debts." &&
        game.action !== "Run like the Devil." &&
        game.action !== "SHOW_COIN" &&
        game.action !== "SUCK_UP" &&
        game.action !== "TELL_GUARDS" &&
        game.action !== "Tell her you're sorry." &&
        game.action !== "Throw your cat at the dog." &&
        game.action !== "Try to reason with the dog." &&
        game.action !== "Try to reason with the mob." &&
        game.action !== "Waddle like God.") {
        possible_outcomes = actions.GET_ATTACKED(game, {});
    } else {
        possible_outcomes = actions[game.action](game, {});
    }
    return raffle.get(possible_outcomes);
};

function add_next_target_suggestion(game) {
    var burnable_places = [
        "church",
    ...
    for (var i = 0; i < burnable_places.length; i++) {
        if (game.places[burnable_places[i]].burnable === false) {
            burned_count += 1;
        } else {
    ...
    if (burned_count >= burnable_places.length / 2 &&
        ...
        game.message += functions.random_choice(burn_messages);
    }
}

function game.arrested() {
    game.lose_all_items();
    game.move_character("prison");
    game.character.person = "other_lunatics";
}

function game.are_or_is() {
    if (game.persons[game.character.person].type === "group") {
        return "are";
    }
    return "is";
}
...
function game.burn() {
    game.places[game.character.place].burnable = false;
    game.places[game.character.place].name =
        "the smoldering remains of " +
        game.places[game.character.place].name;
    game.character.person = null;
    game.message = "You find yourself in " +
    game.places[game.character.place].name + ".";
    add_next_target_suggestion(game);
    if (functions.random_int(3) === 0) {
        spread_fire(game);
    }
}

function burn_a_bunch_of_places(game) {
    ...
    for (var i = 0; i < number_of_places_burned; i++) {
        if (game.places[burnable[i]].burnable === true) {
            game.places[burnable[i]].burnable = false;
            game.places[burnable[i]].name = "the smoldering remains " +
            "of " + game.places[burnable[i]].name;
        }
    }
}

// kills character if he or she doesn't have a four-leaf clover
function game.clover() {
    if (game.character.items["four-leaf clover"] < 1) {
        game.character.is_dead = true;
    } else {
        game.message += " Or at least that's what you think " +
        "would have happened if you didn't have a lucky four-leaf clover.";
    }
}

// conjugates a verb to singular or plural
function conjugate(game, word) {
    if (game.persons[game.character.person].type !== "group") {
        return word + "s";
    }
    return word;
...
function game.decrement_money() {
    switch (game.character.money) {
        case "large_fortune":
            game.character.money = "small_fortune";
            game.message += " You now only have a small fortune.";
            break;
        case "small_fortune":
            game.character.money = "pittance";
            game.message += " You now only have a pittance.";
            break;
        case "pittance":
            game.character.money = NONE;
            game.message += " You now have no money.";
            break;
    }
}
...
function equip_best_weapon(game) {
    var found_weapon_flag = false;
    var keys = Object.keys(items.weapons_map);
    for (var i = 0; i < keys.length; i++) {
        if (game.character.equipped_weapon === "" &&
            game.character.items[keys[i]] > 0) {
            game.character.equipped_weapon = keys[i];
            found_weapon_flag = true;
        } else if (game.character.items[keys[i]] > 0 &&
    ...
    if (game.get_place().outside === false) {
        if (game.get_place().burnable === false &&
            (game.character.place === "church" ||
             ...
             game.character.place === "tavern" ||
             game.character.place === "tower" ||
             game.character.place === "wizard_lab")) {
            return "ground";
        }
        return "floor";
    }
...
function game.get_item(item) {
    if (game.character.items[item] === 0) {
        game.message += " You now have " + functions.a_or_an(item[0]) +
        " " + item + ".";
    } else {
        game.message += " You now have another " + item + ".";
    }
    game.character.items[item] += 1;
}

function game.get_money(money) {
    if (items.money_map[game.character.money].value <
        ...
        game.message +=
        " You still have " +
        items.money_map[game.character.money].name + ".";
    }
}

function game.get_name() {
    return game.persons[game.character.person].name;
}

function game.get_subject() {
    return he_she_they[game.persons[game.character.person].type];
}

function game.get_weapon(weapon) {
    if (game.character.items[weapon] === 0) {
        game.message += " You now have " +
        functions.a_or_an(items.weapons_map[weapon].name[0]) + " " +
        items.weapons_map[weapon].name + ".";
    } else {
        game.message += " You now have another " +
        items.weapons_map[weapon].name + ".";
    }
    game.character.items[weapon] += 1;
    equip_best_weapon(game);
}

function game.get_wizard_teleport_message() {
...
function game.grow_tail() {
    if (game.character.has_tail === false) {
        game.character.has_tail = true;
        game.message = "You grow a " +
            functions.random_choice(["alligator", "beaver", "donkey", "horse",
                                     "monkey", "pig", "rat"]) + " tail.";
    } else {
        game.message = "The potion has no effect.";
    }
}

function game.leave_donkey_behind() {
    if (game.character.items.donkey === 1) {
        game.character.items.donkey = 0;
        game.message += " You have no idea where your donkey went.";
    } else if (game.character.items.donkey > 1) {
        game.character.items.donkey = 0;
        game.message += " You have no idea where your donkeys went.";
    }
}

function game.lose_all_items() {
    for (var item in game.character.items) {
        game.character.items[item] = 0;
    }
    equip_best_weapon(game);
    game.character.money = NONE;
    game.message += " You now have no items.";
    game.message += " You now have no money.";
}

function game.lose_item(item) {
    game.character.items[item] -= 1;
    game.character.items[item] === 0 ?
    game.message += " You no longer have " +
        functions.a_or_an(item[0]) + " " :
    game.message += " You have one less ";
    game.message += item + "." ;
}

function game.move_character(destination) {
    game.character.place = destination;
    game.character.is_threatened = false;
    game.character.person = null;
    if (destination === "docks" ||
        destination === "mermaid_rock" ||
        destination === "pirate_ship") {
        game.message += " You find yourself on ";
    } else if (destination === "gates_of_hell" ||
               destination === "smoking_volcano") {
        game.message += " You find yourself at ";
    } else {
        game.message += " You find yourself in ";
    }
    game.message += game.places[destination].name + ".";
    if (destination === "gates_of_hell" &&
        game.persons.cerberus.alive === true) {
        game.character.person = "cerberus";
        game.message += " A giant three-headed dog is blocking " +
            "your way."; 
    }
}
...
function spread_fire(game) {
    ...
    if (game.places[next_fire].burnable === true) {
        game.message += functions.random_choice([
            " The blaze also takes out ",
            " The fire spreads to ",
            " The fire also destroys ",
            " The flames spread to ",
        ]) + game.places[next_fire].name + ".";
        game.places[next_fire].burnable = false;
        game.places[next_fire].name = "the smoldering remains of " +
            game.places[next_fire].name;
    }
}
...
function game.teleport() {
    var place_list = [];
    for (var place in game.places) {
        if (game.places[place] !==
            game.places[game.character.place] &&
            place !== "gates_of_hell" &&
            place !== "upstairs" &&
            place !== "void") {
...
function game.trash() {
    game.places[game.character.place].trashable = false;
    game.places[game.character.place].name =
